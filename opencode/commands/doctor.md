...
report could be produced at all. On `2`, present the exit code and stderr instead. If the
binary is missing (a checkout that was never built), say so and point at caret's install
script.
...
section (`daemon error: …`) is likewise normal when the daemon is down; present it as-is.
