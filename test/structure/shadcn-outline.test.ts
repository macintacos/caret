...
// ring or border must hide its outline with `outline-hidden`, never `outline-none`.
//
...
// `outline-hidden` paints a transparent outline that forced colors repaints. The
// registry ships `outline-none`, so a re-sync reverts the patch silently — this
// is where it reds. `settings.e2e.ts` proves the painted result on a real field.
//
...
// match no `focus-visible:ring`/`border` utility, so they need no allowlist.
...
const FOCUS_CUE = /(?:^|\s|")focus-visible:(?:ring|border)/;
...
test("no focus-cued vendored component uses outline-none", () => {
  ...
  const violations = paths
    ...
    .map((path) => `${VENDORED_DIR}/${path}: use outline-hidden, which forced colors repaints`);
...
test("the rule reds on a ring-cued outline-none and passes outline-hidden", () => {
  ...
  expect(hidesOutlineUnsafely('"outline-hidden focus-visible:ring-3"')).toBe(false);
});
