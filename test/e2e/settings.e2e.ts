...
test("the volume slider is keyboard-operable, named, and persists", async ({ daemon, page }) => {
  ...
  // Not the toast: under load the presses can straddle the 200ms commit window, so an
  // earlier commit's toast shows while 40 is still pending, and a reload drops that write.
  await expect
    .poll(() => page.evaluate(() => localStorage.getItem("caret.sound.volume")))
    .toBe("0.4");
