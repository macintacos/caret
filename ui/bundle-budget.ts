...
// It lives at the ui/ root beside generate-palette-css.ts, the other build concern the
// config expresses as a plugin, and is split the same way: a pure half (budgetFailure)
// the colocated suite drives with synthetic file lists, and the filesystem half
// (measureDist) that reads a real build. Imported rather than spawned, unlike
// generate-palette-css.ts — that script is spawned because its module graph resolves
// $lib, and this one imports nothing but node builtins and a vite type, so an import
// keeps the budget type-checked against its one consumer.
//
// Total bytes over dist/ rather than a narrower measure, because every payload the alias
// block excludes arrives as a lazily-imported chunk rather than as entry-chunk growth: an
// entry-chunk budget would not see any of it. Total bytes is also the number a human
// reproduces by walking dist/ the way measureDist does below.
 ...
 *
 * The three entries are monotone — breaking one only ever makes dist/ bigger — but their
 * deltas overlap, so none can be derived from the others by subtraction. Each number here
 * is its own build. */
 ...
 *
 * What it protects: two of the three shiki-related entries in ui/vite.config.ts's
 * `resolve.alias` block — the `shiki/wasm` and `@pierre/theme/` entries, which stub the
 * Oniguruma WASM binary and the pierre theme payloads that nothing in caret loads. Before
 * this gate an entry that stopped matching (a renamed specifier after a shiki or
 * @pierre/diffs bump) grew the bundle and failed nothing.
 *
 * `baseline + 164_908`, where the headroom is half of `pierreThemeBreak` rounded to a
 * readable number — half, so ordinary growth has ~161 KiB to move in while that break
 * still overshoots by about as much as it clears. A legitimate raise re-measures and
 * updates MEASURED_BYTES above, not just this number.
 *
 * What it does NOT protect, deliberately: the bare `/^shiki$/` entry. Un-aliasing it moves
 * dist/ by `shikiBarrelBreak` — 10_741 bytes, well inside the noise any usable headroom
 * has to tolerate — because that entry is a behaviour swap (which regex engine, whose
 * themes) rather than a payload it keeps out. No size budget can gate it, so this one does
 * not claim to. */
 ...
 * overage, the largest files, and where to look — following
 * test/structure/dependency-dedupe.test.ts's DUPLICATE_HINT, because a failure that says
 * only "over budget" teaches the reader to raise the number, which is the one outcome this
 ...
 * the same thing caret-palette-css does with its own throw — so `build ui`, `build bin`,
 * `build bundle`, `test e2e`, preflight's own `build ui` task, and a hand-run
 * `bunx vite build` all inherit the gate with no further wiring. `apply: "build"` keeps it
 * off the dev server.
 *
 * A factory rather than an object literal in the config, so `outDir` is a closure local
 * instead of the config's one module-level mutable binding. It is captured in
 * `configResolved` rather than read from `writeBundle`'s `options.dir` — vite resolves
 * that field to this same absolute path — because its type admits undefined.
 *
