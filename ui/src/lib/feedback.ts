 ...
 * stored plan version, so it is these anchor words the agent matches against its
 * own text to find the spot; the elided middle would only add tokens, since the
 * agent already holds the plan. A quote short enough that abbreviation would drop
 * no words is returned whole (whitespace collapsed). */
