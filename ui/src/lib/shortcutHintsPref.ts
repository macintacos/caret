...
//
// Stored as "on"/"off" over the shared definePref helper, which registers the key
// for the `--fresh` reset (prefs.ts) and composes enumLocalStoragePref's never-throw
// fail-safe — a blocked or unavailable localStorage degrades to the default rather
// than breaking the view. Exposed as a boolean since the affordances gate on one.
...
const pref = definePref<"on" | "off">(SHORTCUT_HINTS_KEY, ["on", "off"], "on");
