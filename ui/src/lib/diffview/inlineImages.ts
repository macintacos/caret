...
export function syncInlineImages(root: ParentNode, images: ImageSpanMap): void {
  ...
  // from the map. A line the map still names is left to the settle check below,
  // which is what keeps an unchanged row untouched.
  for (const img of root.querySelectorAll(`[${IMAGE_ATTR}]`)) {
    ...
    if (row === null || !Number.isFinite(line) || !images.has(line)) img.remove();
  }
