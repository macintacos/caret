...
describe("SourceView rendering", () => {
  ...
  test("highlights with caret's registered Shiki theme", async () => {
    ...
    // keyword hue in both color schemes (light #6a37a0 / dark #b982e8, the thistle
    ...
    const tokenStyles = [...(shadow(target)?.querySelectorAll("[style*='--diffs-token']") ?? [])]
      ...
      .join(" ")
      .toLowerCase();
    expect(tokenStyles).toContain("--diffs-token-light:#6a37a0");
    expect(tokenStyles).toContain("--diffs-token-dark:#b982e8");
  });
});
