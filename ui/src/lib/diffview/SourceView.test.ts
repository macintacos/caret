...
describe("SourceView rendering", () => {
  ...
  test("highlights with caret's registered Shiki theme", async () => {
    ...
    // accent color in both color schemes (light #b8440f / dark #ef7d33, the --accent
    ...
    const tokenStyles = [...(shadow(target)?.querySelectorAll("[style*='--diffs-token']") ?? [])]
      ...
      .join(" ")
      .toLowerCase();
    expect(tokenStyles).toContain("--diffs-token-light:#b8440f");
    expect(tokenStyles).toContain("--diffs-token-dark:#ef7d33");
  });
});
