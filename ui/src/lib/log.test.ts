...
import { batchEvents, type LogCapture, logCapture } from "@ui/support/helpers.ts";
...
describe("uiLog buffering and flush", () => {
  ...
  test("a poisoned extra never propagates out of uiLog or flush", () => {
    ...
    let reached = false;
    expect(() => {
      uiLog.info("ui", "still logs", poisoned);
      uiLog.warn("ui", "still logs", poisoned);
      uiLog.error("ui", new Error("x"), poisoned);
      flush();
