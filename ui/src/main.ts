...
import { migrateLegacyTheme } from "$lib/appearance.ts";
...
// transition from. boot() re-seeds from storage first, so the migration above
// lands even though the import that constructed the module ran before it.
appearance.boot();
