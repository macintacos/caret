<script lang="ts">
  ...
  function walk(e: KeyboardEvent, arrow: string): void {
    ...
    repeat.start(e.key, () => {
      ...
      // focus behind it (EXC-1127).
      const sub = stepsOut ? openSubmenu() : null;
  ...
  // The submenu standing open, whichever way it was opened — the caller's answer to
  // "is there something to close before stepping out a crumb". Portalled since
  // EXC-957, so every level is a body-level sibling in mount order and the last open
  // one is the innermost. `data-state` is what excludes a panel still running its
  // exit animation: the node lingers for the animation's 150ms, and a held key ticks
  // every 60ms, so a presence-only test would read a closed submenu as open and stall
  // a hold for two or three ticks.
  function openSubmenu(): HTMLElement | null {
