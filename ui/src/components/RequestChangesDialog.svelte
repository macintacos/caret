<script lang="ts">
  ...
  interface Props {
    ...
    // Lifted into App.svelte and autosaved per review; the dialog is a controlled
    // view over the parent's value.
  ...
  // Scratches are never counted into the tally above (they aren't sent unless
  // Saved), which would otherwise read as a bare contradiction of the "Unsent
  // comments [N]" chip below — so say plainly they won't go unless Saved (EXC-746).
  ...
  // handler carries only caret's own ⌘↵/Ctrl+Enter submit chord, on the body wrapper
  // so it fires wherever focus sits. The MarkdownEditor already intercepts the chord
  // (and preventDefault's it), so guard on !defaultPrevented against a double submit.
  function onKey(e: KeyboardEvent) {
    if (isSubmitChord(e) && !e.defaultPrevented) submit();
  }
</script>

<!-- kind="dialog": Escape + backdrop dismiss, routed to onCancel. The host mounts
     this per open (ModalPresence) and keeps it through the exit. The title is the
     heading bits-ui wires as the accessible name; the eyebrow keeps caret's dialog
     signature. -->
<Modal
  kind="dialog"
  {open}
...
>
  <div class="body" role="presentation" onkeydown={onKey}>
    ...
    <!-- Marking as a draft demotes a comment into "Unsent comments" below, where it
         drops out of the compiled preview and can be Saved back. Only line-anchored
         comments can demote; legacy selection-anchored ones (no line range) offer
         Discard only. -->
    {#if inlineComments.length > 0}
      <section class="inline-comments" aria-labelledby="inline-label">
        ...
        <!-- A real list, so assistive tech gets a count and each row's position
             ...
             enclosing section is already a named region. -->
        <ul class="row-list">
          {#each inlineComments as a (a.id)}
            ...
            <li class="inline-row">
              <Collapsible.Root class="inline-disclosure">
                <!-- The actions ride the head, never the collapsing body, so they
                     show without expanding (the EXC-746 guard). -->
                <div class="row-head">
                ...
                <Collapsible.Content>
                  <div class="row-body">
                    ...
                    <!-- Collapsed by default: the source lines the comment anchors
                         to, so the reviewer can read the code it was written against
                         without leaving the dialog (EXC-762). -->
                    {#if context.length > 0}
    ...
    <!-- Unsent composer drafts ("scratches"): text typed into a line composer but
         never submitted. Not committed comments — the count, empty state and preview
         all exclude them — so they are surfaced here for a conscious Save or Discard.
         The wording stays "unsent", never "Draft" (a created, pending annotation), so
         one never looks like a comment that was actually added. -->
    {#if scratches.length > 0}
...
<!-- Discarding is the destructive one, so it routes through a confirmation; Mark as
     draft and Save do not. The bubble anchors itself to the trigger — this dialog's
     body scrolls, and bits-ui tracks the button through it. Opening a second row's
     bubble dismisses the first as an outside interaction. -->
...
<style>
  /* Widen the modal past the shadcn default. app.css scans only lib/components/ui
     for Tailwind, so a max-w utility written here would never be generated — this
     plain :global rule sets the width directly. Specificity (0,2,0) beats the
     vendored `.sm:max-w-sm` (0,1,0); min() keeps the small-screen inset. */
  :global([data-slot="dialog-content"].rcd-content) {
    ...
    /* Cap the height so a long inline-comment list can't push the modal past the
       screen. Pinning the header/footer rows to auto and the body to 1fr lets ONLY
       the body scroll, keeping the title and the Send/Cancel actions in view; dvh
       tracks the mobile URL-bar viewport. */
    max-height: calc(100dvh - 2rem);
    grid-template-rows: auto minmax(0, 1fr) auto;
  }

  /* The scroll region when the content overflows the capped height. min-height:0
     lets it actually shrink inside the grid row rather than forcing the modal taller.
     It also carries the ⌘↵ keydown (role="presentation": no semantics). */
  .body {
    display: grid;
    /* Without the explicit `0` min, grid items default to min-width:auto
       (min-content) and a long unbroken comment expands the column past the modal,
       forcing a horizontal scroll and shoving the row's action buttons off-screen. */
  ...
  /* Committed inline comments: the solid rail reads "committed" against the scratch
     rows' dashed "started, not sent" below. */
  .inline-comments {
  ...
  /* The row's always-visible top line: the trigger grows, the actions stay put, and
     both are vertically centered so a taller button and a one-line trigger read as
     one row rather than top-aligned and off-kilter. */
  .row-head {
    display: flex;
    align-items: center;
    gap: 0.5rem;
  ...
  /* Unsent-scratch section: the source view's Resume-marker idiom — dashed neutral
     rails, no accent — so an unsent draft never carries the actionable accent the
     dialog reserves for real feedback. */
  .scratches {
    padding: 0.7rem 0.8rem;
  ...
  /* Shared row disclosure trigger (inline + scratch + nested context): a bits-ui
     Collapsible.Trigger (a <button>) reset to read as a plain summary line, with a
     raised-paper hover wash signalling the whole line toggles it. */
  :global(.row-trigger) {
    display: flex;
    align-items: center;
    gap: 0.4rem;
  ...
  /* Children carry their own inset, so a scratch row (whose body has no wrapper) and
     an inline row read with the same left edge. */
  .row-body {
    display: grid;
    grid-template-columns: minmax(0, 1fr);
    gap: 0.35rem;
  ...
  /* On hover each takes its semantic tint — green for the keep, red for the drop —
     so the consequence reads before the click; resting stays the quiet float-chip. */
  :global(.save) {
    transition:
  ...
  /* The disclosure chevron, rotated by the Collapsible's data-state — right when
     closed, down when open. Scoped under .rcd-content so the global selector can't
     reach collapsibles elsewhere. */
  ...
  /* Animated expand/collapse of the disclosure body, driven by bits-ui's own
     measured height var + data-state. The Collapsible's presence machine keeps the
     node mounted until animationend, so this has to be a keyframe animation rather
     than a tween. The one global reduced-motion rule in app.css neutralizes it (a
     per-component block would be dead CSS — see motion.test.ts). */
