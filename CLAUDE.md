...
change where every path is Markdown runs `lint` alone — plus `test` when it touches
Markdown a test reads from disk (`scripts/tasks/dev/fake-plan.md`,
`THIRD_PARTY_LICENSES.md`, `doc/DEVELOPMENT.md`). Anything else runs the full six, as does
an empty or unreadable diff. Read the `start` document's `selection` object before you
report a run as green: `{"narrowed": true, "reason": "…"}` means you proved less than the
whole gate, and `schemaVersion` is `2` precisely because `ok` now means "every task that
ran passed". `--full` forces all six — it is the one preflight flag that works without
`--json` too. Note that `lint` always scans the whole tree even when the gate narrows, so
cross-file link fragments stay checked.
