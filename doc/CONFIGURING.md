...
| `CARET_SUPERVISED`   | —                     | —                | Set to `1` by the generated launchd/systemd unit, naming a process a service supervisor started. `daemon.resident` takes effect only when it is present; it also arms the daemon's own `daemon-stderr.log` rotation, which a supervised start otherwise skips.                                            |
