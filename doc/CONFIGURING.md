...
| `CARET_IDLE_MS`      | `daemon.idle_ms`      | `60000`          | Idle delay before the daemon auto-shuts-down with no reviews. Ignored by a resident daemon — one under a supervisor (`CARET_SUPERVISED=1`) or `caret serve`.                                                                                                                                             |
...
| `CARET_SUPERVISED`   | —                     | —                | Set to `1` by the generated launchd/systemd unit and by `mise run dev`, naming a process started by something that manages its lifetime rather than by a hook. It makes the daemon resident, reported as `resident` by `/api/health` and `/api/diagnostics`; it also arms the daemon's own `daemon-stderr.log` rotation, which a supervised start otherwise skips, and tells `bin/caret-launcher` to open that log itself rather than leave its diagnostics on the terminal.                                            |
