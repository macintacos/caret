...
- **Every hue has a job; everything else is neutral.** The palette recipe
  ...
  one `var()` reader under `ui/src`, so a token can't stay declared for nobody; the four
  content chips still awaiting the ticket that renders them are the one listed exception,
  pinned as read by *nothing*, so the exemption fails the suite the moment it is outlived.
  `--chip-ref` has already outlived its own — it is the resting fill of the file-reference
  chip in `diffview/coreStyles.ts` — and is held to the same floor as every other token.
  ...
  above it for why that pair and not another.
