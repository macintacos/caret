...
| `confirm-popover.e2e.ts` "a click outside the dialog's discard bubble" | A REAL race, and the clearest case of the throttle's blind spot: it survives 90x, yet `--repeat-each 20` at ordinary parallelism reds it 1/20. The arming signal is bits-ui's, not the DOM's: `DismissibleLayer` attaches its document listeners inside `afterSleep(1, …)`, so `toBeVisible()` and the confirm button's focus both resolve inside that window — which is why waiting on the focus still red 3/40. Dropped rather than delayed, because until the bubble registers `isResponsibleLayer` still names the dialog beneath it and a pointerdown on that dialog's own heading is inside it, so no layer dismisses. Measured 4–47ms, still unarmed after `toBeVisible()` in 3 runs of 30. Waited out via `awaitDismissArmed` (EXC-1200), at all three of the file's outside-click sites. Fixed. The same window sits under every other outside-dismiss in the suite — `approve.e2e.ts`'s backdrop `page.mouse.click(5, 5)` most clearly — none of which has been observed flaking; the helper is there when one does. |
