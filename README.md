...
> [!WARNING]
> **Prototype.** caret is an early prototype and may change substantially over the next
...
plan formatter. No `git clone`, no compile step. Where it can't ask — off a terminal — it
installs into every agent it detected.

| Flag                                          | What it does                                            |
| --------------------------------------------- | ------------------------------------------------------- |
| `--target claude`, `--target claude,opencode` | Pin the agents non-interactively, skipping the chooser. |
| `--dry-run`                                   | Preview the run without changing anything.              |
| `--refresh`                                   | Update an existing install.                             |
| `--uninstall`                                 | Remove caret, offering the same chooser in reverse.     |

> [!NOTE]
> caret is **macOS-first**; Linux and Windows are best-effort. See
> [`doc/CONFIGURING.md`](doc/CONFIGURING.md#platform-support) for which paths differ and
...
for pinning a version in OpenCode's config, and for what each agent's install touches.
...
environment variables. Every key, its default, and its environment variable is in
...
[`doc/README.md`](doc/README.md) is the map — it routes you to the reference page that
answers your question.

- **Developing caret?** [CONTRIBUTING.md](CONTRIBUTING.md) — setup, the `mise` workflow,
  and where tests live.
- **Working on caret with a coding agent?** [CLAUDE.md](CLAUDE.md) routes a change to the
  rules-of-the-road that govern it.
