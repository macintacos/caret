...
exists only in code is undiscoverable; the README is the user-facing source of truth.
...
- **README.** Document it in the appropriate `## Configuration` subsection: config-file
  keys as a row in the relevant table under `### Config file` (e.g. the `[logging]`
  table), environment variables as a row in the `### Environment variables` table.
