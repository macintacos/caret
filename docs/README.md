...
Adding a new agent-facing rule? Drop it in `agents/` as its own file and add a route to it
from the digraph in `CLAUDE.md`, so the agent knows when to read it.
