...
// (RUMDL_CONFIG, not .rumdl.toml's). This is the default `doFormat` behind
// formatPlanMarkdown's best-effort envelope (src/plan/markdown.ts): any failure
// here — unsupported platform, offline download, spawn error — throws and is caught
// there, storing the plan raw with one warn, so nothing is ever lost while rumdl is
// absent.
...
export const RUMDL_VERSION = "0.2.73";
...
// caret's installer is bash and its runtime targets are darwin + linux.
const ASSETS: Record<string, RumdlAsset> = {
  ...
  "linux-arm64": {
    ...
    sha256: "f51548adfc71c4982cb5cc239d847bcd4c884e6ac8dc67efd68efd3eea9753a4",
  },
  "linux-x64": {
    ...
    sha256: "dc4aa9bf98f936f667039224a34fc2e26ddf6be3589bf59d63e4d6b038be4f5c",
  },
};

/** Resolve the pinned release asset for a platform/arch (defaults to the current
 * process). Throws on anything not in the map (e.g. win32). */
...
// The formatting-only rumdl config caret ships (EXC-828). Its near-twin
// `.rumdl.toml` governs this repo's own markdown; the two diverge deliberately and
// must not be reconciled.
//
// Exempting link URLs from *measurement* (EXC-931) means a line carrying a link ends
// up physically wider than 90 by roughly the URL's length. Accepted: the plan reader
// scrolls either way, and a sentence chopped around an isolated link reads worse.
//
 ...
 * `rumdl <semver>`), or null when there is nothing runnable there — absent,
 * truncated, not executable, built for another platform: all of them "not the rumdl
 * we pinned". */
 ...
 *
 * `inFlight` coalesces concurrent installs onto one download, and is cleared once
 * it settles — the binary is on disk by then, so the version check is the gate from
 * that point on.
 *
 * A failure instead sets `cooldownUntil` (EXC-1155): retrying per call re-fetches
 * ~5 MB on the critical path of every plan and throws anyway. A window rather than a
 * permanent memo, so a transient failure still recovers without a daemon restart; it
 * gates only the install, so a cached binary reporting RUMDL_VERSION stays reusable
 * throughout.
 *
 ...
 *
 * caret formats every plan with ONE rumdl: `RUMDL_VERSION`, at caret's own path. A
 * binary already sitting there is used only if it reports exactly that version, so
 * an older build cached by a previous caret is replaced. A `rumdl` on PATH is never
 * consulted: plan formatting must not vary with whatever the machine happens to have
 * installed. `CARET_RUMDL_BIN` opts out of the *download*, not out of that thesis —
 * it too is taken only when it reports `RUMDL_VERSION`.
 *
 * Can reject having never called `install`, when a recent failure's
 * `RUMDL_RETRY_COOLDOWN_MS` window is still open; `now` and `state` are injected so
 * a suite can drive that window without sleeping through it.
 *
 * The config is written on every call so it always tracks the current state dir. */
 ...
 * mirroring scripts/tasks/release/rumdl.ts. Throws on any spawn/exit failure. */
