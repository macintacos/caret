...
Read the shared fake-plan fixture at
`${CLAUDE_PLUGIN_ROOT}/scripts/tasks/dev/fake-plan.md` and present its contents
**verbatim** to the user by calling `ExitPlanMode` with that markdown as the `plan`
argument. This is the same fixture `mise run dev` seeds, so the demo and the dev
environment draw their plan from one source of truth. Do **not** do any real work,
